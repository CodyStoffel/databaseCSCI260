...
def updateCourseClick(win, newTextNumber, textCourse, textNumber):
    newNumber = newTextNumber.get("1.0", "end-1c")
    dept = textCourse.get("1.0", "end-1c")
    number = textNumber.get("1.0", "end-1c")
    
    updateCourse(newNumber, dept, number )
    win.destroy()

def updateCourseWindow(root):
    win = Toplevel(root)
    win.title("Update a Course")
    # win.pack()
    ...
    labelNewNumber = Label(leftframe, text = "New Course Number")
    labelNewNumber.pack()
    newTextNumber = Text(rightframe, height=1, width=30)
    newTextNumber.pack()
    ...
    add = Button(rightframe, command = lambda: updateCourseClick(win, newTextNumber, textCourse, textNumber ), text = "Update the Course")
...
update = Button(leftframe, command=lambda:updateCourseWindow(root), text = "Update a Course")
